...
Public Class Form3
    ...
    Dim WINDOW_WARNING As Integer = 10
    ...
    Private Sub Form3_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim updateLoadData As MySqlDataReader = db.runQuery($"SELECT * from {que_table} WHERE QNO = '0'")

        While updateLoadData.Read()
            totalQuestions = Integer.Parse(updateLoadData.GetString("Q").ToString())
            totalMarks = Integer.Parse(updateLoadData.GetString(2))
            totalTime = Integer.Parse(updateLoadData.GetString(3))
        End While

        updateLoadData.Close()
        ...
        Dim countCol As MySqlDataReader = db.runQuery($"SELECT COUNT(*) as data FROM information_schema.columns WHERE table_schema = 'quizfy' AND table_name = '{ans_table}';")
        While countCol.Read()
            colCount = countCol.GetInt64("data")
        End While
        countCol.Close()

        Dim emptyData As String = " "
        For i As Integer = 1 To colCount - 1
            If i.Equals(colCount - 1) Then
                emptyData += ("'0'")
            Else
                emptyData += ("'0',")
            End If
        ...
        Dim regUserForTest As MySqlDataReader = db.runQuery($"INSERT INTO {ans_table} VALUES('{username}',{emptyData})")
        regUserForTest.Close()
